...
def addSong(result, db):
    ...
    if play.find_one({'songs._id':ObjectId(id)}):
        return "Duplicated"
    play.update({'username':result['username'],'list_name':result['playlistName']},{'$push':{'song':{'_id':res['_id'], 'songName':res['title'] }}})
...
def delSong(result, db):
    ...
    if not res:
        return "Wrong Id"
    play.update({'username':username,'list_name':playlistName},{'$pull':{'song': {'songName':res['title'] }}})
...
def fetchPlaylist(result, db):
    ...
    username = result['username']
    playlistName = result['playlistName']
    res = play.find_one({'username':username,'list_namee':playlistName})['song']
    temp = []
    fiRes = []
    for x in res:
